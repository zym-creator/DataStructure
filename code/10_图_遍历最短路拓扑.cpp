...
vector<int> dijkstra(const Graph &graph, int source)
{
    ...
    for (int round = 0; round < graph.size(); round++)
    {
        ...
        for (int i = 0; i < graph.size(); i++)
            if (!used[i] && distance[i] != INT_MAX && (vertex == -1 || distance[i] < distance[vertex]))
                vertex = i;
        if (vertex == -1)
            break;
        used[vertex] = true;
        ...
        for (int i = 0; i < (int)edges.size(); i++)
        {
            ...
            if (edges[i].weight >= 0 && candidate < distance[next])
...
int main()
{
    ...
    cout << "BFS: ";
    printVector(bfs(graph, 0));
    cout << "最短距离: ";
    printVector(dijkstra(graph, 0));
    vector<int> order;
    cout << "拓扑序: ";
